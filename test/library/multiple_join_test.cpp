...
static int check_node_list_equality(int start_index)
{
	...
	list_for_each_entry_continue(n1, &node_list.nlist, nlist) {
		for (int i = 0, index = start_index % 6; i < nb_nodees;
		     i++, index = (index + 1) % 6) {
...
TEST(multiple_joins, try_join_the_same_node)
{
	...
	for (int i = 1; i <= 5; i++) {
		...
		if (!pid) {
			...
			switch (i) {
			case 1:
				join_DSM(creator.host, creator.port, LOCALHOST,
					 joiner1.port);
				break;
...
TEST(multiple_joins, try_join_in_a_queue)
{
	...
	for (int i = 1; i <= 5; i++) {
		...
		if (!pid) {
			...
			case 2:
				join_DSM(joiner1.host, joiner1.port, LOCALHOST,
					 joiner2.port);
				break;

			case 3:
				join_DSM(joiner2.host, joiner2.port, LOCALHOST,
					 joiner3.port);
				break;

			case 4:
				join_DSM(joiner3.host, joiner3.port, LOCALHOST,
					 joiner4.port);
				break;

			default:
				join_DSM(joiner4.host, joiner4.port, LOCALHOST,
					 joiner5.port);
				break;
			}
